...
class StatewiseAdapter(private val context: Context, private var dataArrayList: ArrayList<StatewiseData>, private val districtDataList: ArrayList<DistrictData>) : RecyclerView.Adapter<StatewiseAdapter.ViewHolder>() {
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        //Confirmed Cases
        holder.mBinder.tvConfirmedCount.text = Utils.getFormattedNumber(statewiseData.confirmed.toDouble(), false)
        holder.mBinder.tvConfirmedDeltaCount.text = String.format("[+%s]", Utils.getFormattedNumber(statewiseData.deltaconfirmed.toDouble(), false))

        //Active Cases
        holder.mBinder.tvActiveCount.text = Utils.getFormattedNumber(statewiseData.active.toDouble(), false)

        //Death Cases
        holder.mBinder.tvDeathCount.text = Utils.getFormattedNumber(statewiseData.deaths.toDouble(), false)
        holder.mBinder.tvDeathDeltaCount.text = String.format("[+%s]", Utils.getFormattedNumber(statewiseData.deltadeaths.toDouble(), false))

        //Recovered Cases
        holder.mBinder.tvRecoveredCount.text = Utils.getFormattedNumber(statewiseData.recovered.toDouble(), false)
        holder.mBinder.tvRecoveredDeltaCount.text = String.format("[+%s]", Utils.getFormattedNumber(statewiseData.deltarecovered.toDouble(), false))
